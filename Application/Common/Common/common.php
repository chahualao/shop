...
function sendSMS($mobile,$content)
{
    ...
    //执行发送操作
    $result = httpRequest($config['url'],'POST',$data);;
    //把返回的数据解析成数据对象
    $message = simplexml_load_string($result);

    //组装短信平台的返回数据
    return array(
        'message_description'=>$message->errordescription,
        'message_resultcode'=>$message->resultcode,//0为发送成功
        'message_taskcode'=>$message->taskcode,
    );  
}
/**
*   模拟提交数据
...
*   @return string  返回数据
**/
function virtual_post($url,$data){
    $ch = curl_init();  //创建一个会话
    $cookie_file = '';  //cookie文件
    if(file_exists(dirname(__FILE__)."/cookie_".md5(basename(__FILE__)).".txt")){
		...
		if($val['prom_type'] == 3)
        {
			...
			if($prom){
				...
				if($coupon && $coupon['createnum']>0){					                                        
                    ...
                    //满足条件为 剩余不为零且订单的总金额必须大于优惠券的最低消费金额
                    if(($remain > 0) && ($order['total_amount'] > $coupon['condition']))                                            
                    {
...
function get_goods_promotion($goods_id,$user_id=0){
	...
	if($goods['prom_type'] == 3){//优惠促销
		...
		if(!empty($prominfo)){
			...
			}elseif($prominfo['type']==3){//送优惠卷

                //此处不能添加，不然加入购物车的时候就已经送优惠券了
                
                //代金卷id
                //$coupon_id = $prominfo['expression'];
                //查询出来代金卷
                //$coupon = M('coupon')->where("id = $coupon_id and use_end_time > ".time()." and ((createnum - send_num ) > 0 or createnum = 0) ")->select();

                //单个商品的价格必须大于次代金券限制的数量
                /*if($coupon['condition']<$goods['shop_price']){
                    M('coupon_list')->add(array('cid'=>$coupon_id,'type'=>0,'uid'=>$user_id,'send_time'=>time()));
                    M('Coupon')->where("id = {$coupon_id}")->setInc('send_num'); // 优惠券领取数量加一 
                }else{
                    //没有满足条件，不送优惠券
                    //todo...
                }*/
                
                //这个优惠卷是针对订单的 订单的总价大于限制的价格就行了
                //M('coupon_list')->add(array('cid'=>$coupon_id,'type'=>0,'uid'=>$user_id,'send_time'=>time()));
                //M('Coupon')->where("id = {$coupon_id}")->setInc('send_num'); // 优惠券领取数量加一 
...
function get_goods_category_tree(){
	$result = array();
	$cat_list = M('goods_category')->where("is_show = 1")->order(array('sort_order'=>'desc','id'=>'asc'))->cache(true,10)->select();//所有分类
