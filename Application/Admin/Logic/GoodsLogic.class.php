...
class GoodsLogic extends RelationModel
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
            ...
            $str .="<td><input id='check_spec_flag' name='item[$item_key][price]' value='{$keySpecGoodsPrice[$item_key][price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
        ...
        <script>
            //增加规格选择限制
            $(function(){
                $('#check_spec_flag').bind('input propertychange', function() {  
                    $('.check_spec_btn_tag').each(function(){
                        //获取规格名
                        var spec_name = $(this).siblings('td').html();
                        var btns = $(this).children('button');
                        
                       if(!$(btns).hasClass('btn-success')){
                            alert('['+spec_name+']不能漏选');
                            return false;
                        }
                    })
