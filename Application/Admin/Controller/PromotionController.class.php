...
class PromotionController extends BaseController {
	...
	public function prom_goods_save()
	{
        ...
        $data['jump_url'] = empty($data['jump_url']) ? 'javascript:;' : $data['jump_url'];

        $data['goods_id']  = is_array($data['goods_id']) ? implode(',', $data['goods_id']) : '';
		...
		$this->success('更新促销活动成功',U('Promotion/prom_goods_list'));
	}
    ...
    //推荐到首页
    public function prom_goods_star()
    {
        $id = I('id');
        $prom = M('prom_goods');                
        $rs = $prom->find($id);
        ...
        $prom->id =  $id;
        $prom->star = ($rs['star'] == 0) ? 1 : 0;
        $r = $prom->save();
        if ($r) {
            $this->success('推荐到首页成功',U('Promotion/prom_goods_list'));
        }else{
            $this->error('推荐失败',U('Promotion/prom_goods_list'));
        }
