    ...
    public function addEditGoods()
    {
        ...
        if (($_GET['is_ajax'] == 1) && IS_POST) {
            ...
            if (!$Goods->create(NULL, $type))// 根据表单提交的POST数据创建数据对象
            {
                ...
                if (isset($_POST['zuhe_goods_id'])) {
                   $Goods->zuhe_goods_id = implode(',', $_POST['zuhe_goods_id']);
                }else{
                    $Goods->zuhe_goods_id = '';
                }
        ...
        //商品标签
        $system_config = M('config')->where("name='goods_tags'")->find();
        $goods_tags = explode('|', $system_config['value']);
        $this->assign('goods_tags',$goods_tags);
        //组合优惠商品 从所有商品中选择
        if(!empty($goodsInfo['zuhe_goods_id'])){
            $tmp = explode(',', $goodsInfo['zuhe_goods_id']);
            $map['goods_id']=array('in',$tmp);
            $zuhe_goods = M('Goods')->where($map)->where('is_on_sale=1')->select();
            //echo M()->getLastSql();
        }else{
             $zuhe_goods = array();
        }

        $this->assign('zuhe_goods', $zuhe_goods);
