...
class AdminController extends BaseController {
    ...
    public function adminHandle(){
        ...
        $isAdmin = isAdmin();
        //非超级管理员不能新建超级管理员账户
        if(!$isAdmin && $data['role_id'] ==1){
            $this->error("非超级管理员不能新建超级管理员账户！",U('Admin/Admin/admin_info'));
        }
    	if($data['act'] == 'add'){
    		unset($data['admin_id']);    		
    		$data['add_time'] = time();
	...
	public function supplier_info()
	{
		...
		$admin = M('admin')->field('admin_id,user_name')->where($map)->select();
