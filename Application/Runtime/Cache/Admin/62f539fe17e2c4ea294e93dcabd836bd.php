                    ...
                    <div class="tab-content col-md-10">                 	  
                        <div class="tab-pane active" id="tab_tongyong">                           
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <td class="col-sm-2">促销活动副标题：</td>
                                    <td class="col-sm-8">
                                        <input type="text" class="form-control" name="sub_name" id="sub_name" value="<?php echo ($info["sub_name"]); ?>" >
                                ...
                                <tr>
                                    <td>适合用户范围：</td>
                                    <td>
                                    	<?php if(is_array($level)): foreach($level as $key=>$vo): ?><input type="checkbox" <?php if(strripos($info['group'],$vo['level_id']) !== false): ?>checked<?php endif; ?> name="group[]" value="<?php echo ($vo["level_id"]); ?>"><?php echo ($vo["level_name"]); endforeach; endif; ?>
                                    </td>        
                                </tr>
                                <tr>
                                    <td>促销链接：</td>
                                    <td>
                                        <input type="text" class="form-control" id="jump_url" name="jump_url" value="<?php echo ($info["jump_url"]); ?>">   
                                    </td>
                                </tr>   
                                <tr>
                                    <td>选择商品:</td>
                                    ...
                                    <td colspan="2"> 
                                       <div class="form-group">
                                       		<table class="table table-bordered">
									            ...
									            <tbody id="goods_list">
									            <?php if(is_array($prom_goods)): foreach($prom_goods as $key=>$vo): ?><tr>
...
$("#prom_type").on("change",function(){
 ...
 $md5_key = md5("select * from __PREFIX__coupon where type=0"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from __PREFIX__coupon where type=0"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($v[id] == $info[expression]): ?>selected<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; ?></select></td>';
	      break;
	    }
	    case 4:{
...
<style type="text/css">
    #tab_tongyong tr td{vertical-align: middle;}
</style>
</body>
</html>