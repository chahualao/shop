				             ...
				             <div class="form-group">
                                 ...
                                 <a class="btn btn-default" href="<?php echo U('/Mobile/Goods/categoryList',array('edit_ad'=>1));?>" >手机分类页</a>&nbsp;&nbsp;&nbsp;&nbsp;
                                    ...
                                    <select class="form-control" name="intro" onchange="if(this.value != 0) location.href='/index.php?m=Home&c=Channel&a=index&edit_ad=1&id='+this.value;">
                                        ...
                                        <?php
 $md5_key = md5("select * from `__PREFIX__goods_category` where `level` = 1"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__goods_category` where `level` = 1"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><option value="<?php echo ($v[id]); ?>"><?php echo ($v[name]); ?></option><?php endforeach; ?>
                                    </select>
