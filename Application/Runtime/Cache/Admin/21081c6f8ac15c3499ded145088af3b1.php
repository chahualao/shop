     ...
     <table class="table table-bordered table-hover">
         ...
         <tbody id="goos_table">
             <?php if(is_array($userList)): $i = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td class="text-left">
                        <?php if( $list[nickname] == null): if( $list[oauth] == 'weixin'): ?>微信用户<?php endif; ?>
                            <?php if( $list[oauth] == 'weibo'): ?>微博用户<?php endif; ?>
                            <?php if( $list[oauth] == 'alipay'): ?>支付宝用户<?php endif; ?>
                            <?php if( $list[oauth] == 'qq'): ?>QQ用户<?php endif; ?>
                           <?php else: ?>
