                ...
                <div class="form-group">
                  <select name="cat_id" id="cat_id" class="form-control">
                    <option value="">所有分类</option>
                        <?php if(is_array($categoryList)): foreach($categoryList as $k=>$v): ?><option value="<?php echo ($v['id']); ?>" <?php if($v[id] == $cat_id): ?>selected<?php endif; ?> ><?php echo str_pad('',($v[level] * 5),'-',STR_PAD_LEFT); ?> <?php echo ($v['name']); ?></option><?php endforeach; endif; ?>
                  </select>
                </div>
                <div class="form-group">
          ...
          <div id="ajax_return"> 
			    <div class="table-responsive">
			        <table class="table table-bordered table-hover" id="goos_table">
			            ...
			            <tbody>
			            <?php if(is_array($goodsList)): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><!--如果有商品规格-->
                            <?php if($list['spec_goods'] != null): if(is_array($list['spec_goods'])): $i = 0; $__LIST__ = $list['spec_goods'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$spec_goods): $mod = ($i % 2 );++$i;?><tr>
