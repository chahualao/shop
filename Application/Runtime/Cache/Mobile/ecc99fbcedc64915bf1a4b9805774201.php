...
.load-more-out{margin-top: 10px;}
		...
		<section class="filtrate_term" id="product_sort" style="width: 100%; background: rgb(255, 255, 255); position: fixed; top: 51px; display: block;">
			<ul>
				<li class="<?php if(($_GET[sort] == '') or ($_GET[sort] == 'is_new')): ?>on<?php endif; ?>">
					<a href="<?php echo urldecode(U('Mobile/Goods/search',array_merge($filter_param,array('sort'=>'is_new')),''));?>">最新<span class="arrow_up"></span><span class="arrow_down"></span></a>
				</li>
				<li class="<?php if($_GET[sort] == 'shop_price'): ?>on<?php endif; ?>">
					<a href="<?php echo urldecode(U('Mobile/Goods/search',array_merge($filter_param,array('sort'=>'shop_price','sort_asc'=>$sort_asc)),''));?>">价格<span class="arrow_up"></span><span class="arrow_down"></span></a>
				</li>
				<li class="<?php if($_GET[sort] == 'sales_sum'): ?>on<?php endif; ?>">
					<a href="<?php echo urldecode(U('Mobile/Goods/search',array_merge($filter_param,array('sort'=>'sales_sum')),''));?>">销量<span class="arrow_up"></span><span class="arrow_down"></span></a>
				</li>
				<li class="">
  ...
  <?php else: ?> 
   <?php if(is_array($goods_list)): foreach($goods_list as $k=>$vo): ?><li>
			...
			<!-- <div id="getmore" style="font-size:.24rem;text-align: center;color:#888;padding:.25rem .24rem .4rem; clear:both;padding-top: 18px;padding-bottom: 60px;">
