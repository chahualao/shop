...
<div class="goods_nav hid" id="menu">
			...
			<div class="main" id="user_goods_ka_1" style="display:block;">
				<div class="banner">
					<div id="slider" class="slider" style="overflow: hidden; visibility: visible; list-style: none; position: relative;">
						...
						<div id="pagenavi">
							<?php if(is_array($goods_images_list)): foreach($goods_images_list as $k=>$pic): ?><a href="javasscript:void(0);" <?php if($k == 0): ?>class="active"<?php endif; ?>></a><?php endforeach; endif; ?>
						</div>
					</div>
				</div>
				...
				<form name="buy_goods_form" method="post" id="buy_goods_form">
					<div class="product_info">
						...
						<dl class="goods_price">
							...
							<dt> <span id="goods_price">￥<?php echo ($group_buy_info["price"]); ?>元</span><em>限时团购</em><strong  id="leftTime">24:13:52</strong><!--<font>价格：￥<?php echo ($goods["market_price"]); ?>元</font>--> </dt>
						</dl>
						<script>
                            // 倒计时
                            function GetRTime2(){
						...
						<ul class="price_dottm">
							<li style=" text-align:left">折扣：<?php echo ($group_buy_info["rebate"]); ?>折</li>
							<li><?php echo ($commentStatistics["c0"]); ?>人评价</li>
							<li style=" text-align:right"><?php echo ($group_buy_info[virtual_num] + $group_buy_info[buy_num]); ?>人已参团</li>
						</ul>
					</div>
 ...
 $md5_key = md5("select * from `__PREFIX__prom_order` order by  id desc limit 5"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__prom_order` order by  id desc limit 5"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><ul class="youhui_list1" style="margin-top:0px;">
					...
					<!-------商品属性-------->
					<?php if($filter_spec != ''): ?><section class="search_ka">
							<!---属性---->
					...
					<script type="text/jscript">
						function click_search() {
							var search_ka = document.getElementByClassName("search_ka");
			...
			<div class="main" id="user_goods_ka_2" style="display:none">
				...
				<section class="index_floor">
					...
					<ul class="xiangq">
						<li>
							<p>开始时间：</p>
							<span><?php echo (date('Y-m-d H:i',$group_buy_info["start_time"])); ?></span>
							<li>
					</ul>
				</section>
			</div>
