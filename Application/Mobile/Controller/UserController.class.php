...
class UserController extends MobileBaseController
{
    ...
    */
    ...
    {   
        ...
        if (IS_POST) {
            ...
            if (check_mobile($username) && tpCache('sms.regis_sms_enable')) {
                ...
                $check_code = $logic->check_validate_code($code,$username);//由原来的数据库验证改为session验证
    ...
    //ajax 地址信息
    public function ajax_edit_addtess(){
    ...
    **/
    public function mobileinfo(){
        ...
        if (IS_POST) {
            ...
            //验证码判断
            if($data['verify'] != session('validate_code.code')){
                $rdata['error']='验证码错误';
                $this->ajaxReturn($rdata,'json');
            }
            //防止收到验证码后篡改手机
            if($data['mobile'] != session('validate_code.sender')){
                $rdata['error']='该手机号和接收验证码手机号不一致';
                $this->ajaxReturn($rdata,'json');
            }
            if($act == 'bind'){
                //是否已经被绑定
                $user = get_user_info($data['mobile'],2);
                if ($user) {
                    $rdata['error']='该手机已被绑定,请先解绑';
                    $this->ajaxReturn($rdata,'json');
                }
                $userLogic = new UsersLogic();
                if (!$userLogic->update_info($this->user_id, array('mobile'=>$data['mobile']))){
                     $rdata['error']='绑定手机失败';
                }else{
                    $rdata['status']=1;
                    $rdata['msg']='绑定成功';
                ...
                $userLogic = new UsersLogic();
                if (!$userLogic->update_info($this->user_id, array('mobile'=>''))){
                     $rdata['error']='解绑失败';
                }else{
                    $rdata['status']=1;
                    $rdata['msg']='解绑成功';
    ...
    public function ajax_post_data(){
        ...
        I('post.sex') ? $data['sex']=I('post.sex') : null;
        I('post.nickname') ? $data['nickname']=I('post.nickname') : null;
        I('post.mobile') ? $data['mobile']=I('post.mobile') : null;
        I('post.email') ? $data['email']=I('post.email') : null;
        if (!empty($data['email'])) {
            $e = M('users')->where("email = '{$data['email']}' and user_id != {$this->user_id}")->count();
            if($e){
                $return_data['error']='email已被使用';
                $return_data['msg']='邮箱已被使用';
            }     
        }
        if (!empty($data['mobile'])) {
            $m = M('users')->where("mobile = '{$data['mobile']}' and user_id != {$this->user_id}")->count();
            if($m){
                $return_data['error']='mobile已被使用';
                $return_data['msg']='手机号已被使用';
            }     
        }
        //更新
        $userLogic = new UsersLogic();
    ...
    */
    ...
    public function send_validate_code()
    {
        ...
        $send = I('send');
        $logic = new UsersLogic();
        $this->ajaxReturn($logic->send_validate_code($send, $type));
    }
    ...
    public function order_confirm()
    {
        ...
        if (!$data['status'])
            $this->error($data['msg'],U('order_list',array('type'=>'WAITRECEIVE')));
        else
            $this->success($data['msg'],U('order_list'));
    }
